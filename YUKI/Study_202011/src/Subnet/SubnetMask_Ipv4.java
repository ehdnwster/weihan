...
import progresql.Dao;
import progresql.NetInfoEntity;

public class SubnetMask_Ipv4 {
	...
	public static void main(String[] args)  {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			try {
				...
				Dao dao=new Dao();
				try {
					NetInfoEntity item=new NetInfoEntity();
					item.setInput_data(list.get(i));
					item.setIp_type("C");
					item.setSubnetmask("255.255.255.0");
					item.setInsert_date("now()");
					item.setInsert_user("Yuki");
					item.setUpdate_date("now()");
					item.setUpdate_user("Yuki");
					dao.insert_netinfo(item);
	...
	private static void subnetIpAddress(String subNetLength, String ip_sub1, String ip_sub2, String ip_sub3) {
		...
		if (ipSub1 < 127) {
			...
			for (int h = 0; h < idList.size(); h++) {
				...
				System.out.println("broadcast ip=" + ipPrefix + endIp + ".0.0" + "\n");
			}
			// B类IP地址
		} else if (ipSub1 < 192) {
			...
			for (int h = 0; h < idList.size(); h++) {
				...
				System.out.println("broadcast ip=" + ipPrefix + endIp + ".0" + "\n");
			}
			// C类IP地址
		} else if (ipSub1 < 224) {
