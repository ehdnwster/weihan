...
public class SubnetMask_Ipv4_Convet_Ipv6 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			try {
				...
				// 将每个调取IP地址(元素)按照“/”拆分为含有2个元素的字符串数组
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private static String calculate(String[] ipSet) {
		...
		if (ret == "") {
			...
			if (Integer.parseInt(ip[0]) < 224) {
				...
				ret = "当前子网ip的ipv6格式IP地址计算结束.";

			}
			// D类IP地址判断
			else if (Integer.parseInt(ip[0]) < 240) {
				System.out.println("当前IP地址为D类IP地址.");
				ret = "v6IpNetAddress is " + v6IpConventer(subNet_1) + ".";
			}
			// E类IP地址判断
			else {
				System.out.println("当前IP地址为E类IP地址.");
				ret = "v6IpNetAddress is " + v6IpConventer(subNet_1) + ".";
			}
		}
	...
	private static void subnetIpAddress(String subNetLength, String ip_sub1, String ip_sub2, String ip_sub3) {
		...
		if (ipSub1 < 127) {
			...
			for (int h = 0; h < idList.size(); h++) {
				...
				// System.out.println("net ip=" + ipPrefix + startIp);
				System.out.println("netIp_v6 is " + v6IpConventer(ipPrefix + startIp));
				for (int avIp = startIp + 1; avIp < endIp; avIp++) {
					System.out.println("availabelIp_v6 is " + ipPrefix + avIp);
				}
				System.out.println("broadcastIp_v6 is " + ipPrefix + endIp + ".0.0");
			...
			for (int h = 0; h < idList.size(); h++) {
				...
				System.out.println("broadcastIp_v6 is " + v6IpConventer(ipPrefix + endIp) + ".0");
			}
			// C类IP地址
		} else if (ipSub1 < 224) {
			...
			for (int h = 0; h < idList.size(); h++) {
				...
				System.out.println("netIp_v6 is " + v6IpConventer(ipPrefix + startIp));
				for (int avIp = startIp + 1; avIp < endIp; avIp++) {
					// System.out.println("availabel ip=" + ipPrefix + avIp);
					System.out.println("availabelIp_v6 is " + v6IpConventer(ipPrefix + avIp));
				}
				// System.out.println("broadcast ip=" + ipPrefix + endIp + "\n");
